...
for s in recordlist:
    if s.attributes['type'].value == "HKQuantityTypeIdentifierHeartRate":
        ...
        try:
            ...
            year, month, date, hour, minute, float(s.attributes['min'].value) * 60,
            float(s.attributes['max'].value) * 60, float(s.attributes['average'].value) * 60,
            float(s.attributes['recordCount'].value)))
        except:
            # try to parse the non-consolidated ones
            print("%s,%s,%s,%s,%s,%d,%d,%d,%d,+" % (
            year, month, date, hour, minute, float(s.attributes['value'].value) * 60,
            float(s.attributes['value'].value) * 60, float(s.attributes['value'].value) * 60, 1))
